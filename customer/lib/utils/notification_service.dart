...
class NotificationService {
  ...
  Future<void> initInfo() async {
    ...
    if (request.authorizationStatus == AuthorizationStatus.authorized || request.authorizationStatus == AuthorizationStatus.provisional) {
      ...
      const AndroidNotificationChannel orderChannel = AndroidNotificationChannel(
        'booking_channel',
        'Bookings',
        description: 'Channel for new booking notifications',
    ...
    await FirebaseMessaging.instance.subscribeToTopic("mytaxi-customer");
  }
  ...
  void display(RemoteMessage message) async {
    ...
    try {
      bool isBooking = message.data['isBooking'] == 'true';
      AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
        isBooking ? 'booking_channel' : 'default_channel',
        isBooking ? 'Bookings' : 'General',
        channelDescription: 'Booking Notifications',
        importance: Importance.max,
        priority: Priority.high,
        playSound: true,
        sound: isBooking ? const RawResourceAndroidNotificationSound('booking_notification') : null,
      );

      DarwinNotificationDetails iosDetails = DarwinNotificationDetails(
        presentSound: true,
        sound: isBooking ? 'booking_notification.wav' : null,
      );

      NotificationDetails notificationDetailsBoth = NotificationDetails(android: androidDetails, iOS: iosDetails);
