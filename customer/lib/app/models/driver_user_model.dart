...
import 'package:customer/app/models/verify_driver_model.dart';
...
class DriverUserModel {
  ...
  DriverUserModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json['verifyDocument'] != null) {
      verifyDocument = List<VerifyDocument>.from(
        (json['verifyDocument'] as List).map((x) => VerifyDocument.fromJson(x)),
      );
    } else {
      verifyDocument = [];
    }
    ...
    zoneId = List<String>.from(json['zoneId'] ?? []);
  ...
  Map<String, dynamic> toJson() {
    ...
    if (verifyDocument != null) {
      data['verifyDocument'] = List<dynamic>.from(verifyDocument!.map((x) => x.toJson()));
    }
