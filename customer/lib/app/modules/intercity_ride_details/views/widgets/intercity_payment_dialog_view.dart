...
class InterCityPaymentDialogView extends StatelessWidget {
  const InterCityPaymentDialogView({super.key});
            ...
            children: [
              ...
              Padding(
                padding: EdgeInsets.only(
                    top: 4, bottom: MediaQuery.of(context).padding.bottom, left: ((Responsive.width(100, context) - 208) / 2), right: ((Responsive.width(100, context) - 208) / 2)),
                child: RoundShapeButton(
                  ...
                  onTap: () async {
                    if (controller.selectedPaymentMethod.value == Constant.paymentModel!.cash!.name) {
                      ...
                      controller.interCityModel.value.paymentType = controller.selectedPaymentMethod.value;
                      controller.interCityModel.value.paymentStatus = false;
                      await FireStoreUtils.setInterCity(controller.interCityModel.value).then((value) async {
                        if (value == true) {
                          DriverUserModel? receiverDriverModel = await FireStoreUtils.getDriverUserProfile(controller.interCityModel.value.driverId.toString());
                      ...
                      ShowToastDialog.showToast("Payment method updated");
                      Navigator.pop(context);
                    }
                  },
