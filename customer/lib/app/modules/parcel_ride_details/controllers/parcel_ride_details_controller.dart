...
class ParcelRideDetailsController extends GetxController {
  ...
  void listenToInterCityRideDetails() {
    FireStoreUtils.getParcelRideDetails(bookingId.value).listen((ParcelModel? model) {
      parcelModel.value = model ?? ParcelModel();
    });
    if (selectedPaymentMethod.value == '') {
      selectedPaymentMethod.value = parcelModel.value.paymentType.toString();
    }
  }
  ...
  Future<void> completeOrder(String transactionId) async {
    bool isPaymentStatus = parcelModel.value.paymentStatus ?? false;
    ShowToastDialog.showLoader("Please wait".tr);
    // bookingModel.value.paymentStatus

    parcelModel.value.paymentType = selectedPaymentMethod.value;
    if (parcelModel.value.paymentType == Constant.paymentModel!.cash!.name) {
      parcelModel.value.paymentStatus = selectedPaymentMethod.value == Constant.paymentModel!.cash!.name ? false : true;
    }

    //if payment type cash -------->send notification to driver
    WalletTransactionModel transactionModel = WalletTransactionModel(
        id: Constant.getUuid(),
        amount: Constant.calculateParcelFinalAmount(parcelModel.value).toString(),
        createdDate: Timestamp.now(),
        paymentType: selectedPaymentMethod.value,
        transactionId: transactionId,
        userId: parcelModel.value.driverId,
        isCredit: true,
    ...
    await FireStoreUtils.setWalletTransaction(transactionModel).then((value) async {
      if (value == true) {
        await FireStoreUtils.updateOtherUserWallet(amount: Constant.calculateParcelFinalAmount(parcelModel.value).toString(), id: parcelModel.value.driverId!);
      }
    }).catchError((error) {
    ...
    if (Constant.adminCommission != null && Constant.adminCommission!.active == true && num.parse(Constant.adminCommission!.value!) > 0) {
      WalletTransactionModel adminCommissionWallet = WalletTransactionModel(
          id: Constant.getUuid(),
          amount:
              "${Constant.calculateAdminCommission(amount: ((double.parse(parcelModel.value.subTotal ?? '0.0')) - (double.parse(parcelModel.value.discount ?? '0.0'))).toString(), adminCommission: parcelModel.value.adminCommission)}",
          createdDate: Timestamp.now(),
          paymentType: "Wallet",
          transactionId: parcelModel.value.id,
          isCredit: false,
          type: Constant.typeDriver,
          userId: parcelModel.value.driverId,
          note: "Admin commission Debited",
          adminCommission: parcelModel.value.adminCommission);
    ...
    log('====---33--> selecte payment status ${parcelModel.value.paymentStatus}');
    parcelModel.value.paymentStatus = isPaymentStatus;

    await FireStoreUtils.setParcelBooking(parcelModel.value);
    ShowToastDialog.closeLoader();
    log('====---444--> selecte payment status ${parcelModel.value.paymentStatus}');

    DriverUserModel? receiverUserModel = await FireStoreUtils.getDriverUserProfile(parcelModel.value.driverId.toString());
    if (receiverUserModel != null) {
      Map<String, dynamic> playLoad = {"bookingId": parcelModel.value.id};
      await SendNotification.sendOneNotification(
        type: "order",
        token: receiverUserModel.fcmToken.toString(),
        title: 'Payment Received',
        body: 'Payment Received for Ride #${parcelModel.value.id.toString().substring(0, 5)}',
  ...
  Future<void> walletPaymentMethod() async {
    ...
    await FireStoreUtils.setWalletTransaction(transactionModel).then((value) async {
      if (value == true) {
        await FireStoreUtils.updateUserWallet(amount: "-${Constant.calculateParcelFinalAmount(parcelModel.value).toString()}").then((value) async {
          await getProfileData();
          parcelModel.value.paymentStatus = true;
          await FireStoreUtils.setParcelBooking(parcelModel.value);
        });
      }
    });
      ...
      if (value != null) {
       ...
       await Get.to(PayStackScreen(
          ...
          if (value) {
            ...
            completeOrder(value['transaction_id']);
            setTransactionLog(isCredit: true, transactionId: value['transaction_id'], transactionLog: value, amount: finalAmount.toString());
          } else {
            ShowToastDialog.showToast("Payment UnSuccessful!!".tr);
          }
        });
