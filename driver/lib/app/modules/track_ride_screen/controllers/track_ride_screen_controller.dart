...
class TrackRideScreenController extends GetxController {
  ...
  void getArgument() {
    ...
    if (args == null || args is! Map || args['bookingModel'] == null) {
      developer.log("Missing bookingModel arguments in TrackRideScreenController");
    ...
    _bookingSub = FirebaseFirestore.instance.collection(CollectionName.bookings).doc(bookingModel.value.id).snapshots().listen((bookingSnap) {
      ...
      final bool statusChanged = _lastBookingStatus != bookingModel.value.bookingStatus;
  ...
  Future<void> openGoogleMap() async {
    ...
    try {
      ...
      if (bookingModel.value.bookingStatus == BookingStatus.bookingPlaced ||
          ...
          bookingModel.value.bookingStatus == BookingStatus.driverAssigned) {
        googleMapsUrl =
            "https://www.google.com/maps/dir/?api=1&origin=${driverUserModel.value.location!.latitude},${driverUserModel.value.location!.longitude}&destination=${bookingModel.value.pickUpLocation!.latitude},${bookingModel.value.pickUpLocation!.longitude}&travelmode=driving";
      } else if (bookingModel.value.bookingStatus == BookingStatus.bookingOngoing || bookingModel.value.bookingStatus == BookingStatus.bookingOnHold) {
        googleMapsUrl =
            "https://www.google.com/maps/dir/?api=1&origin=${driverUserModel.value.location!.latitude},${driverUserModel.value.location!.longitude}&destination=${bookingModel.value.dropLocation!.latitude},${bookingModel.value.dropLocation!.longitude}&travelmode=driving";
      }
