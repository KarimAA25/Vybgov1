...
class ChatScreenController extends GetxController {
  ...
  Future<void> getData(String receiverId) async {
    final driverProfile = await FireStoreUtils.getDriverUserProfile(FireStoreUtils.getCurrentUid());
    if (driverProfile != null) {
      senderUserModel.value = driverProfile;
    }
    final userProfile = await FireStoreUtils.getUserProfile(receiverId);
    if (userProfile != null) {
      receiverUserModel.value = userProfile;
    }
  ...
  Future<void> sendMessage() async {
    ...
    await SendNotification.sendOneNotification(
        type: "chat", token: receiverUserModel.value.fcmToken.toString(), title: senderUserModel.value.fullName.toString(),isBooking: false, body: message.value, payload: playLoad);
