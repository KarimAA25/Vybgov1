...
class TrackParcelRideScreenController extends GetxController {
  // ================= MAP CONTROLLERS =================
  ...
  void getArgument() {
    ...
    _bookingSub = FirebaseFirestore.instance.collection(CollectionName.parcelRide).doc(parcelModel.value.id).snapshots().listen((bookingSnap) {
      ...
      parcelModel.value = ParcelModel.fromJson(bookingSnap.data()!);

      // ---------- STATUS CHANGE HANDLING ----------
      final bool statusChanged = _lastBookingStatus != parcelModel.value.bookingStatus;

      if (statusChanged) {
        _lastBookingStatus = parcelModel.value.bookingStatus;
  ...
  Future<void> getDirections({bool forceReroute = false}) async {
    try {
      if (parcelModel.value.id == null) return;
      final driver = driverUserModel.value.location;
      final pickup = parcelModel.value.pickUpLocation;
      final drop = parcelModel.value.dropLocation;
      ...
      if (parcelModel.value.bookingStatus == BookingStatus.bookingPlaced || parcelModel.value.bookingStatus == BookingStatus.driverAssigned) {
        origin = PointLatLng(pickup.latitude!, pickup.longitude!);
        destination = PointLatLng(drop.latitude!, drop.longitude!);
      } else if (parcelModel.value.bookingStatus == BookingStatus.bookingAccepted) {
        origin = PointLatLng(driver.latitude!, driver.longitude!);
        destination = PointLatLng(pickup.latitude!, pickup.longitude!);
      } else if (parcelModel.value.bookingStatus == BookingStatus.bookingOngoing || parcelModel.value.bookingStatus == BookingStatus.bookingOnHold) {
        origin = PointLatLng(driver.latitude!, driver.longitude!);
        destination = PointLatLng(drop.latitude!, drop.longitude!);
      } else {
        return;
      ...
      final result = await polylinePoints.value.getRouteBetweenCoordinates(
        request: PolylineRequest(origin: origin, destination: destination, mode: TravelMode.driving, optimizeWaypoints: true),
      );
  ...
  void _updateGoogleMarkers(LocationLatLng pickup, LocationLatLng drop, LocationLatLng? driver) {
    markers.clear();
    if (parcelModel.value.bookingStatus == BookingStatus.bookingPlaced ||
        parcelModel.value.bookingStatus == BookingStatus.driverAssigned ||
        parcelModel.value.bookingStatus == BookingStatus.bookingAccepted) {
    ...
    if (parcelModel.value.bookingStatus == BookingStatus.bookingPlaced ||
        parcelModel.value.bookingStatus == BookingStatus.driverAssigned ||
        parcelModel.value.bookingStatus == BookingStatus.bookingOngoing ||
        parcelModel.value.bookingStatus == BookingStatus.bookingOnHold) {
      markers['drop'] = Marker(
  ...
  Future<void> fetchOsmRoute(latlang.LatLng start, latlang.LatLng end) async {
    try {
    ...
    try {
      ...
      if (_activeOSMRoutePoints.isNotEmpty) {
        ...
        if (deviated && DateTime.now().difference(_lastOSMReroute) > osmRerouteCooldown) {
          ...
          await fetchOsmRoute(origin, destination);
        } else {
  ...
  Future<void> openGoogleMap() async {
    ...
    try {
      ...
      if (parcelModel.value.bookingStatus == BookingStatus.bookingPlaced ||
          ...
          parcelModel.value.bookingStatus == BookingStatus.driverAssigned) {
        googleMapsUrl =
            "https://www.google.com/maps/dir/?api=1&origin=${driverUserModel.value.location!.latitude},${driverUserModel.value.location!.longitude}&destination=${parcelModel.value.pickUpLocation!.latitude},${parcelModel.value.pickUpLocation!.longitude}&travelmode=driving";
      } else if (parcelModel.value.bookingStatus == BookingStatus.bookingOngoing || parcelModel.value.bookingStatus == BookingStatus.bookingOnHold) {
        googleMapsUrl =
            "https://www.google.com/maps/dir/?api=1&origin=${driverUserModel.value.location!.latitude},${driverUserModel.value.location!.longitude}&destination=${parcelModel.value.dropLocation!.latitude},${parcelModel.value.dropLocation!.longitude}&travelmode=driving";
      }
